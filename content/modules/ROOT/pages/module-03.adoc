...
- Navigate back to the + sign again, we need the *ClusterRoleBinding* object to grant the OpenShift GitOps service account access to the *openshift-gitops-policy-admin* ClusterRole object. Click *Create* once complete
...
- Under *Sync Policy* uncheck *Automatically sync when cluster state changes* 
...
image::03-route-patient.png[link=self, window=blank, width=100%, Application Route]

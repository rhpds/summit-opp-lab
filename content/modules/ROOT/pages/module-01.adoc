== Module 01: Red Hat Advanced Cluster Management for Kubernetes
...
- To get started, switch over to the OpenShift Console https://console-openshift-console.apps.cluster-pbpsm.pbpsm.sandbox1327.opentlc.com
- Remember that the login is *kubeadmin* and the password is: *dd3aF-yuHGT-janMV-Aq7gr*

Once you have logged in to OpenShift Console, navigate to the *Cluster* drop down menu and then select *All Clusters*. 

This will bring you to the RHACM interface. 

image::001-local-cluster.png[Cluster Selection Menu]
...
image::002-cluster-view.png[View of Clusters Listed]
...
image::003-create-cluster.png[Create Cluster]
...
image::004-aws-credentials.png[AWS Credentials]
...
Name your cluster *demo-cluster*, and select *default* for the cluster set. Lastly select the most recent release image *OpenShift 4.14.6*. 

Click on *Next* to continue.

image::005-create-cluster-details.png[Create Cluster Details]
...
image::006-create-cluster-nodepools.png[Create Cluster NodePools]

The next screen allows you to configure networking type to use and it's associated  variables. 

Click on *Next* to proceed.

image::007-create-cluster-networking.png[Create Cluster Networking]
...
image::008-create-cluster-summary-create.png[Create Cluster Summary Page]
...
image::009-view-new-cluster.png[View New Cluster]
...
image::010-labels.png[Cluster Labels]
...
image::011-application-topology.png[Application Topology]
...
image::012-application-route.png[Application Route]
...
image::013-grc-diagram.png[Governance, Risk, Compliance Diagram]
...
image::014-policy-toggle.png[Display the YAML]
...
- Under the actions dropdown, select *Enforced*. This will enforce the policy, wait until the green checkmark is displayed.

image::015-enforce-governance-policy.png[Enforce the Governance Policy]
...
* Branch: main
- Verify that *Deploy only to local cluster* is selected using label name equals any of local-cluster and cluster set to be default.
- Verify all the information is correct, click *Create*.

It will take a few minutes to deploy the application, *Click on the Topology Tab* to view and verify that *all of the circles are green*.

image::016-governance-topology.png[Governance Topology]
...
image::017-policies-list.png[Governance Policies List]
...
image::018-policies-enforce-red.png[Enforce the Policy]
...
image::019-policies-enforce-green.png[Policy Enforced]

...
image::201-lab-environment.png[link=self, window=blank, Lab Environment]
...
image::202-cert-warning.png[link=self, window=blank, Certificate Warning]

image::203-login-page.png[link=self, window=blank, Login Page]
...
image::204-admin-login.png[link=self, window=blank, Admin Login]

- Ensure you maintain access to the console and keep your tab open for the future lab sections.

image::205-rhacs-dashboard.png[link=self, window=blank, RHACS Dashboard]
...
image::206-numbered-dashboard.png[link=self, window=blank, Numbered Dashboard]
...
image::207-top-bar.png[link=self, window=blank, Top Bar]
...
image::209-search-syntax.png[link=self, window=blank, Search Syntax]
...
image::210-nav-menu.png[link=self, window=blank, Navigation Menu]
...
image::211-dashboard-center.png[link=self, window=blank, Center Dashboard]
...
image::212-three-dashboards.png[link=self, window=blank, Three Dashboard Sections]
...
image::213-dashboard-filter.png[link=self, window=blank, Dashboard Filter]

image::214-dashboard-dropdown.png[link=self, window=blank, Dashboard Drop-down]
...
+
image::215-vuln-dashboard.png[link=self, window=blank, Vulnerability Management Dashboard]
+
...
+
image::216-top-risky.png[link=self, window=blank, Riskiest Deployments]
+
...
+
image::217-policy-buttons.png[link=self, window=blank, Top Policy Buttons]
+
...
+
image::218-riskiest-images.png[link=self, window=blank, Riskiest Images]
+
...
+
image::219-visa-proc.png[link=self, window=blank, Visa Processor Image]
+
...
+
image::220-search-bar.png[link=self, window=blank, Search Bar]
+
You can move on to the next section only when the dashboard displays the image below.
+
image::221-image-info.png[link=self, window=blank, Image Info]
...
+
image::222-fix-vulns.png[link=self, window=blank, Fixable Vulnerabilities]
+
- Above the Image Findings section, click on the *Dockerfile* tab:
+
image::223-dockerfile.png[link=self, window=blank, Dockerfile View]
+
...
+
image::224-riskiest-images2.png[link=self, window=blank, Riskiest Images]
+
. Search for the log4shell vulnerability using its CVE number (*CVE-2021-44228*)
+
image::225-log4shell-search.png[link=self, window=blank, log4shell Search]
+
...
+
image::226-risk-panel.png[link=self, window=blank, Risk Panel]
+
...
+
image::227-log4shell-risk.png[link=self, window=blank, Log4Shell Risk]
+
. Click on the log4shell deployment and review the risk indicators.
+
image::228-log4shell-info.png[link=self, window=blank, Log4Shell Info]
+
...
+
image::229-visa-processor.png[link=self, window=blank, Visa Processor Info]
+
...
+
image::230-policy-mgmt-dashboard.png[link=self, window=blank, Policy Management Dashboard]
+
. Filter through the policies to find *Ubuntu Package Manager Execution* or use the search bar to select by *category*.
+
image::231-policy-search.png[link=self, window=blank, Policy Management Search]
+
. Once you have found the policy *Ubuntu Package Manager Execution*, click on it to learn more.
+
image::232-policy-mgmt-details.png[link=self, window=blank, Policy Management Details]
+
...
+
image::233-network-graph-menu.png[link=self, window=blank, Network Graph Menu]
+
...
+
image::234-network-namespace.png[link=self, window=blank, Network Namespace/Deployments]
+
...
+
image::235-backend-atlas.png[link=self, window=blank, Backend-Atlas Analysis]
...
image::236-add-to-baseline.png[link=self, window=blank, Add to Baseline]
...
In the complete network graph, deployment names are not visible. Details of their flows and baselines are available when you zoom in on them. You can also view network flow details by hovering over the connecting lines.

. Scroll or pinch to zoom in on the *payments* namespace. As you zoom in, the namespace boxes show the individual deployment names.
...
+
image::237-visa-processor-aflows.png[link=self, window=blank, Visa Processor Anomalous Flows]
+
NOTE: Your *Anomalous Flows* list may be different.
+
...
+
image::238-network-policy-generator.png[link=self, window=blank, Network Policy Generator]
+
. Click the *Generate and simulate network policies* button.
+
image::239-gen-sim-pol.png[link=self, window=blank, Generate and Simulate Network Policies]
+
. It will generate YAML for applying a new firewall policy based on the current environment.
+
image::240-net-pol-yaml.png[link=self, window=blank, Network Policy YAML]
...
In our next next section we will demonstrate this functionality through examples of policy violations.
...
+
image::241-microservices-demo.png[link=self, window=blank, Microservices Demo]
+
. From the left, navigate to the *Compliance* page.
. In the upper right, click *SCAN ENVIRONMENT*:
+
image::242-compliance-dashboard.png[link=self, window=blank, Compliance Dashboard]
+
...
+
image::243-filter-pci-64.png[link=self, window=blank, Filter Microservices Demo 64%]
+
. After you note the current compliance level, return to the Network Graph page. Next, you generate some network policies to improve the compliance score on the PCI benchmark.
. At the top, use the filter bar to narrow the view to Namespace: *microservices-demo*. The network graph changes focus to only the *microservices-demo* namespace.
. Click Network Policy Simulator and then Generate and simulate network policies:
+
image::244-network-pol-sim.png[link=self, window=blank, Network Policy Simulator]
+
. Click *Apply Network Policies*.
. Refresh and expect to see that the deployments in the *microservices-demo* namespace are *blue*, indicating that they have network policies applied. The PCI compliance score increases for the *microservices-demo* namespace, because those network policies meet the requirements for isolation of retail applications that handle cardholder data.
. Navigate back to *Compliance* and click *Scan Environment*. After the compliance scan completes, you can see that the *PCI score* has increased.
...
+
image::245-filter-pci-79.png[link=self, window=blank, Filter Microservices Demo 79%]
...
+
image::246-settings-enabled.png[link=self, window=blank, Verify Settings Enabled]
+
image::247-dynamic-config.png[link=self, window=blank, Dynamic Configuration]
+
...
+
image::248-image-noscan.png[link=self, window=blank, Images with No Scans]
+
...
image::248a-violation-log.png[link=self, window=blank, Violation Log]
...
. Click the *Actions* drop down and enable the policy. At build time, the roxctl binary can be used to **preview** the Deployment before actually attempting to create it in a Kubernetes cluster.
...
+
image::249-enforce-runtime.png[link=self, window=blank, Enforce Runtime Policy]
+
. Click save.
...
+
image::250-violations.png[link=self, window=blank, Violations Menu]
+
...
image::251-resolve-violation.png[link=self, window=blank, Resolve Violations]
